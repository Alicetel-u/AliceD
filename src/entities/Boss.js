...
export class Boss {
    constructor(x, y, tileSize, config) {
        ...
        this.timer = 0;
        this.defeated = false;
        this.lastAttackState = null; // Track the previous attack to prevent immediate repetition
            ...
            if (ai) {
                ...
                if (this.timer > interval) {
                    ...
                    if (pool && pool.length > 0) {
                        // NO-REPEAT LOGIC: Filter out the last attack if alternatives exist
                        let effectivePool = pool;
                        if (pool.length > 1 && this.lastAttackState) {
                            const filtered = pool.filter(p => {
                                const stateName = (typeof p === 'object') ? p.state : p;
                                return stateName !== this.lastAttackState;
                            });
                            if (filtered.length > 0) effectivePool = filtered;
                        }

                        let selectedState = effectivePool[0];
                        if (typeof effectivePool[0] === 'object') {
                            const totalWeight = effectivePool.reduce((sum, a) => sum + (a.weight || 0), 0);
                            let r = Math.random() * totalWeight;
                            for (const attack of effectivePool) {
                                r -= (attack.weight || 0);
                                if (r <= 0) {
                            ...
                            selectedState = effectivePool[Math.floor(Math.random() * effectivePool.length)];
                        }

                        this.state = selectedState;
                        this.lastAttackState = selectedState; // Store for next time
                        this.timer = 0;
                    }
                }
            } else {
    ...
    updateDarkWaves(dt, player, env) {
        for (let i = this.waves.length - 1; i >= 0; i--) {
            ...
            const isJudgmentRay = w.type === 'JUDGMENT_RAY';
            const isWarmingUp = isJudgmentRay && w.warmup && (w.life > (1.8 - w.warmup));

            if (isWarmingUp) {
                const targetX = player.x + player.width / 2 - w.width / 2;
                w.x += (targetX - w.x) * (dt * 12.0);
            } else if (isJudgmentRay && !w.fired) {
                w.fired = true;
            ...
            if (
                ...
                if (w.type === 'GOD_LASER_BEAM') {
                    // STAGE 5: Multi-hit God Laser - Rapid damage
                    // Apply damage with very short invulnerability (0.1s) to create "da-da-da" effect
                    const currentTime = Date.now();
                    if (!player.lastGodLaserHit || currentTime - player.lastGodLaserHit > 80) { // 80ms interval
                        player.game.takeDamage(this, 0.08, true); // No knockback, tiny cooldown
                        player.lastGodLaserHit = currentTime;

                        // Intense hit effects
                        this._spawnZapEffect(env, player);
                        if (player.game.camera.shake) player.game.camera.shake(0.1, 15);

                        // Extra blood/impact particles for "da-da-da" feel
                        if (env) {
                            for (let k = 0; k < 3; k++) {
                                env.particles.push({
                                    ...
                                    life: 20, size: 8, color: '#ff3333'
                                });
                            }
                        }
                    }
                } else if (w.type === 'JUDGMENT_RAY') {
                    // STAGE 5: Vertical Ray - Avoid by Jumping (Safe in air, lethal on ground)
                    const isWarmingUp = w.warmup && (w.life > (1.8 - w.warmup)); // Ray life starts at 1.8
                    if (player.grounded && !isWarmingUp) {
                        // Rapid multi-hit if staying on ground
                        const currentTime = Date.now();
                        if (!player.lastJudgmentHit || currentTime - player.lastJudgmentHit > 80) {
                            player.game.takeDamage(this, 0.08, true); // Continuous damage
                            player.lastJudgmentHit = currentTime;

                            this._spawnZapEffect(env, player);
                            if (player.game.camera.shake) player.game.camera.shake(0.1, 10);
                        }
                    }
    ...
    drawDarkWaves(ctx, camera) {
        ...
        this.waves.forEach(w => {
            ...
            if (isLaserType) {
                ...
                const isJudgmentRay = w.type === 'JUDGMENT_RAY';
                const isWarmingUp = isJudgmentRay && w.warmup && (w.life > (1.8 - w.warmup));
                // Control master transparency for warmup phase
                const masterAlpha = isWarmingUp ? 0.5 : 1.0;

                // 1. Wide Outer Glow
                ctx.shadowColor = isWarmingUp ? 'rgba(255, 255, 255, 0.6)' : '#9b59b6';
                ctx.shadowBlur = 30 * pulse;
                ctx.globalAlpha = (isWarmingUp ? 0.4 : 0.3) * pulse * masterAlpha;
                ctx.fillStyle = isWarmingUp ? '#fff' : '#8e44ad';
                ...
                const grad = isVertical
                    ? ctx.createLinearGradient(wx, wy, wx + w.width, wy)
                    : ctx.createLinearGradient(wx, wy, wx, wy + w.height);

                if (isWarmingUp) {
                    grad.addColorStop(0, 'rgba(155, 89, 182, 0)');
                    grad.addColorStop(0.5, `rgba(155, 89, 182, ${0.4 * pulse})`);
                ...
                ctx.fillStyle = '#fff';
                ctx.globalAlpha = (0.6 + Math.random() * 0.2) * masterAlpha;
                if (isVertical) {
                ...
                // 4. Energy Streaks
                ctx.strokeStyle = isWarmingUp ? 'rgba(255, 255, 255, 0.3)' : '#d6a2e8';
                ctx.lineWidth = 2;
                ctx.globalAlpha = 0.4 * masterAlpha;
                for (let i = 0; i < 3; i++) {
                    ...
                    if (isVertical) {
                        ...
                        ctx.moveTo(lineX, wy); ctx.lineTo(lineX, wy + w.height);
                    } else {
                        const lineY = wy + (w.height * (0.2 + i * 0.3));
                        ctx.moveTo(wx, lineY); ctx.lineTo(wx + w.width, lineY);
                    }
                    ctx.stroke();
                }
                ctx.setLineDash([]);

                // 5. Scrolling Energy Particles
                ctx.globalAlpha = 0.8 * masterAlpha;
                for (let i = 0; i < 8; i++) {
                    const px = isVertical ? wx + (Math.sin(time * 10 + i) * (w.width * 0.3)) + w.width / 2 : wx + ((time * 1500 + i * 200) % (w.width + 100));
                    const py = isVertical ? wy + ((time * 1500 + i * 200) % (w.height + 100)) : wy + (Math.sin(time * 10 + i) * (w.height * 0.3)) + w.height / 2;
