...
export class Game {
                ...
                if (this.dialogueFlags && this.state === 'PLAYING') {
                    ...
                    if (!this.dialogueFlags.opening) {
                        ...
                        if (this.dialogueManager.hasDialogue(key)) {
                            ...
                            this.dialogueFlags.opening = true; // Skip if non-existent
                        }
                    }
    ...
    async nextStage() {
        ...
        // Play Stage BGM (Fallback to NONE if not defined)
        const stageBgm = (this.currentStageConfig && this.currentStageConfig.bgm) ? this.currentStageConfig.bgm : 'NONE';
        this.audio.playBGM(stageBgm);
    ...
    getTransparencyKey(name) {
        ...
        const keys = {
            // Pre-processed assets no longer need runtime transparency keys
        };
    ...
    loop(timestamp) {
        ...
        if (this.state === 'ENDING') {
            ...
            // Wait for stage title animation (approx 2.0s visible part of 2.5s anim)
    ...
    startDialogue(key, onComplete) {
        ...
        const currentChar = this.characterManager.getCurrentCharacter();
        // Check if character is Kanon (assuming ID is 'kanon' or similar based on CharacterManager)
        // In previous steps, we haven't seen CharacterManager details, but let's assume 'kanon'.
        // Actually, let's allow loosely.
