export class Level {
    ...
    draw(ctx, assets, camera) {
        ctx.save();
        ctx.globalAlpha = 1.0; // Ensure full opacity for tiles
    ...
    drawHeavenBlock(ctx, assets, type, x, y, size, c, r) {
        ...
        // --- 1. Base Layer (Ensure 100% Opacity) ---
        // Fill based on block type before drawing anything else
        ctx.fillStyle = (type === 'B') ? '#f1c40f' : '#ffffff';
        ctx.fillRect(x, y, size, size);
        ...
        if (type === '#' && floorImg) {
            ctx.drawImage(floorImg, x, y, size, size);
        } else if (type === 'D' && wallImg) {
            ctx.drawImage(wallImg, x, y, size, size);
        } else if (type === 'B' && pillarImg) {
            ctx.drawImage(pillarImg, x, y, size, size);
        }

        // --- 3. Mandatory Decorative Layer (Heavenly Borders) ---
        // Always draw borders to fix "transparency" (blending into background)
        ctx.strokeStyle = '#f1c40f'; // Solid Gold
        ctx.lineWidth = 2;
