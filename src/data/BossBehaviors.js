...
export const BossBehaviors = {
    ...
    'JUDGMENT_RAY': (boss, { dt, player, camera, screenWidth }) => {
        ...
        if (boss.timer > 2.0) { // Increased delay from 1.0 to 2.0
            ...
            if (boss.rayTimer <= 0) {
                boss.waves.push({
                    ...
                    vy: 0,
                    type: 'JUDGMENT_RAY',
                    life: 1.8, // Slightly longer life
                    warmup: 0.6 // New property: not lethal during this time
                });
                boss.rayTimer = 1.3; // Slightly more interval
            }
            if (boss.timer > 6.0) { // Extended total duration due to longer delay
                boss.state = 'BATTLE';
                boss.timer = 0;
