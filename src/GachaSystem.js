export class GachaSystem {
    ...
    renderStockIcons() {
        const stockContainer = document.getElementById('item-stock');
        stockContainer.innerHTML = ''; if (this.game.state === 'HOME') return;
        Object.keys(this.collection).forEach(id => {
            const count = this.collection[id]; const item = this.items.find(i => i.id == id);
            if (!item) return;
            const div = document.createElement('div'); div.className = 'stock-icon'; div.style.background = this.getRarityColor(item.rarity); div.style.position = 'relative'; div.innerHTML = this.getIconHtml(item, 20); div.title = item.name;
            if (count > 1) {
                const badge = document.createElement('span'); badge.style.position = 'absolute'; badge.style.bottom = '1px'; badge.style.right = '1px'; badge.style.fontSize = '8px'; badge.style.fontWeight = 'bold'; badge.style.color = '#fff'; badge.style.textShadow = '1px 1px 2px #000'; badge.innerText = `x${count}`; div.appendChild(badge);
            }
